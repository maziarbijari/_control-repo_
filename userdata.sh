...
function prepare_os_packages {
  ...
  if [[ ${OS} = *"Ubuntu"* ]]; then
    ...
    mkdir -p /opt/aws/bin
    ln $(which cfn-signal) /opt/aws/bin/cfn-signal
    pip install awscli
  else
    yum -y update
    yum install -y git
...
function set_aws_settings {
  ...
  # set global settings
  export PUPPETSERVER="puppetsrv-cp2ozuvukeppmiaw.ap-southeast-2.opsworks-cm.io"
